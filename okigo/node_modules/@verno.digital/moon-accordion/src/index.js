...
class MoonAccordion {
  ...
  #accordion = ({block, others}) => {
    const id = block.getAttribute('data-accordion-id')
    ...
    const handlerClick = () => {
      const isMulty = block.hasAttribute('data-accordion-multy')

      if (!this.#isOpen(block) && !isMulty) {
        this.#closeOthers(others, block.getAttribute('data-accordion-id'))
      }
  ...
  #closeOthers(others, id) {
    others.forEach((inst) => {
      inst.id === id && inst.close()
    })
  }
